"""
Compare: RetNet vs RetNet+Based (TaylorExp kernel) vs RetNet+ReBased.
...
Run:
    python -m zoology.launch zoology/zoology/experiments/based_kernel/run_retnet_based.py -p
"""
import uuid
import numpy as np
...
sweep_name = "retnet_based_kernel_" + sweep_id
...
def make_config(name, mixer_dict, lr):
    return TrainConfig(
        ...
        logger=LoggerConfig(
            ...
            tags=["retnet-based-kernel", "based-kernel", name],
        ),
...
MIXERS = {
    # RetNet + TaylorExp (Based kernel) — thay XPOS bằng TaylorFeatureMap
    "retnet_based": dict(
        name="zoology.mixers.retnet_based.RetNetBased",
